...
#Exact solution
def u_exact(x,y,t,Pe,alpha):
	return exp(-t/Pe)*(.5*cos(x-t) + .5*cos(y-alpha*t))
...
for k in range(0,3):
	...
	while t<tend-1e-6 :
		...
		for i in range(0,Nx):
			for j in range(0,Ny):
				u_arr[i][j]=u_old[i][j]*(1.-2.*(lx + ly)) + u_old[period(i-1,Nx)][j]*(lx+ax) +u_old[period(i+1,Nx)][j]*(lx-ax)+u_old[i][period(j-1,Ny)]*(ly+ay)+u_old[i][period(j+1,Ny)]*(ly-ay)
	...
	for i in range(0,Nx):
		for j in range(0,Ny):
			f[i][j] =u_arr[i][j]- u_exact(i*dx,j*dy,t,Pe,alpha)
			f[i][j]=f[i][j]**2
